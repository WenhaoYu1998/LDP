...
public:
    ...
    bool draw_line(GridMap &grid_map, Vec3b value, string target_frame, vector<Point2i>& bbox);
    bool base2map(GridMap &grid_map, Vec3b value, string target_frame, const Point2d& base, Point2i& map);
    // 在grid_map上截取机器人视角图
    bool view(GridMap& grid_map);
    vector<int> CreateRandomNums(int min,int max, int num);

    void trajectory_visual(vector<float> v, vector<float> w, GridMap &grid_map, Vec3b value, string target_frame);
    void global_path_visual(vector<int> global_path_x, vector<int> global_path_y, GridMap &grid_map, Vec3b value, string target_frame);
    ...
    vector<Point2d> traj_points_;
    vector<Point2i> visual_traj_;

    vector<Point2d> global_path_points_;
    vector<Point2i> visual_global_path_;

    int robot_back_state;
    ...
    double control_hz_;
    double step_hz_;
    int refresh_num_episode_;
    double run_dis_episode_;
    // 记录每个episode，小车实际的行使轨迹点
    vector<double> run_trajectory_points_episode_x_;
    vector<double> run_trajectory_points_episode_y_;
    double cur_velocity_;
