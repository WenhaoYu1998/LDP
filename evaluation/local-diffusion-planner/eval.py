...
@click.option('-y', '--yaml', required=True)
@click.option('-m', '--model_type', default='diffusion') # 'diffusion', 'gmm', 'ibc'
@click.option('-e', '--extro_condition', default='gpath') # 'none', 'gpath', 'gmap'
@click.option('-d', '--device', default='cuda:0')

def main(checkpoint, output_dir, yaml, model_type, extro_condition, device):
    ...
    env_cfg = read_yaml(yaml) # 'envs/cfg/circle.yaml'
    ...
    if extro_condition == 'gpath':
        frame_global_path = deque([], maxlen=To)
    elif extro_condition == 'gmap':
    ...
    for step in range(1000000):
        ...
        for i in range(len(action_v[:4])):
            ...
            action_v[i] = max(min(action_v[i], 0.6), -0.6)
            action_w[i] = max(min(action_w[i], 0.785), -0.785)
