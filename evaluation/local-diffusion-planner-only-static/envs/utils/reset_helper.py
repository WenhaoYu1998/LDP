...
class EnvPos:
    ...
    def reset(self, map, path, dir, count):
        obs_msg = self.reset_obs()
        robots_msg, peds_msg, flag = self._reset_robot_ped(map, path, dir, count)
        while not flag:
            robots_msg, peds_msg, flag = self._reset_robot_ped(map, path, dir, count)
    ...
    def _get_robo_ped_module_size(self, size, shape):
        ...
        for i in range(len(size)):
            ...
            if s2 == 'circle':
                ...
                o.append(math.sqrt(s1[0] ** 2 + s1[2] ** 2))
            elif s2 == 'leg':
                o.append(s1[-1]+s1[-2])
            else:
                raise ValueError
    ...
    def _reset_robot_ped(self, map, path, dir, count):
        ...
        robots_msg, peds_msg = [], []
        flag = True
        temp = np.load(f"{dir}/{map[:-4]}.npy", allow_pickle=True)[(count//1)%1]
        start, end = np.array(temp)[0], np.array(temp)[-1]

        # 保证行人的目标点远离机器人的目标点，防止出现行人停在机器人目标点的位置
        for p in range(self.cfg['robot']['total'], robo_ped_num):
            for r in range(self.cfg['robot']['total']):
                if((self.target_poses[p][0] - self.target_poses[r][0]) ** 2 + \
                    (self.target_poses[p][1] - self.target_poses[r][1]) ** 2 < 6.0):
                        flag = False
                        break
                        break
                if((self.init_poses[p][0] - self.target_poses[r][0]) ** 2 + \
                    (self.init_poses[p][1] - self.target_poses[r][1]) ** 2 < 6.0):
                        flag = False
                        break
                        break
        for i in range(self.cfg['robot']['total']):
            ...
            robot.init_pose.position.x = start[0]*0.1
            # robot.init_pose.position.y = self.init_poses[i][1]
            robot.init_pose.position.y = start[1]*0.1
            ...
            robot.goal.x = end[0]*0.1
            robot.goal.y = end[1]*0.1
