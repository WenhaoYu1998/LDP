...
//     // Map_Node() : parent_x(-1), parent_y(-1), f(FLT_MAX), g(FLT_MAX), h(FLT_MAX) {}
//     };
...
public:
    ...
    static const int ROW = 1066;
    static const int COL = 1066;
    ...
    // vector<pair<int, int>> tracePath(Map_Node nodeDetails[][COL], pair<int, int> dest);
    // vector<pair<int, int>> aStarSearch(Mat &grid, pair<int, int> src, pair<int, int> dest);

    void view_ped();//obs_map->ped_map tmp no muti-thread
    void view_robot(bool is_draw_traj);
    void view_agent(bool is_draw_traj);
