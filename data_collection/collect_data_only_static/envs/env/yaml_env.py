...
class ImageEnv(gym.Env):
    ...
    def _init_static_param(self, cfg):
        ...
        self.data_path = cfg['global_map']['data_path']
        self.map_dir_name = cfg['global_map']['map_dir_name']
        self.path_dir_name = cfg['global_map']['path_dir_name']
        self.map = os.listdir(f"{self.data_path}/{self.map_dir_name}/")
        self.path = os.listdir(f"{self.data_path}/{self.path_dir_name}/")
        self.count = 0
    ...
    def _init_req(self):
        ...
        self.init_req.is_record_video = self.cfg['record_video']
        self.init_req.video_episode = self.cfg['video_episode']
        self.init_req.video_path = self.cfg['video_path']
        self.init_req.video_name = self.cfg['video_name']
    ...
    def _reset_req(self):
        ...
        now_map = self.map[(self.count//1)%len(self.map)]
        # print(now_map)
        self.init_req.env.map_file = get_map_file(f"{self.map_dir_name}/{now_map}")
        rospy.wait_for_service(self.init_env_service_name)
        init_env_srv = rospy.ServiceProxy(self.init_env_service_name, InitEnv)
        init_env_srv(self.init_req)
        ...
        if self.cfg_type == 'yaml':
            ...
            self.reset_req.obstacles, self.reset_req.robots, self.reset_req.peds = self.env_pose.reset(now_map, self.path, f"{self.data_path}/{self.path_dir_name}/", self.count)
    ...
    def _step_req(self, actions: List[ContinuousAction]):
        for i in range(self.robot_total):
            ...
            if self.dones[i] == 0:
                ...
                self.step_req.robots[i].traj_v = action.traj_v
                self.step_req.robots[i].traj_w = action.traj_w
                self.step_req.robots[i].global_path_x = action.global_path_x
                self.step_req.robots[i].global_path_y = action.global_path_y
            else:
                self.step_req.robots[i].alive = False
                self.step_req.robots[i].v = 0
                self.step_req.robots[i].w = 0
                self.step_req.robots[i].traj_v = []
                self.step_req.robots[i].traj_w = []
                self.step_req.robots[i].global_path_x = []
                self.step_req.robots[i].global_path_y = []
    ...
    def _trans_cv2_sensor_map(self, view_map):
        ...
        return img_data.astype('float16')
    
    def _trans_cv2_global_map(self, global_map):
        cv_image = self.bridge.imgmsg_to_cv2(global_map, desired_encoding='8UC1')
        return cv_image

    def _get_states(self, robot_states):
        vec_states, sensor_maps, lasers, ped_infos, ped_maps, is_collisions, is_arrives, distances, refresh_num_episode, run_dis_episode = [], [], [], [], [], [], [], [], [], []
        run_trajectory_points_episode_x, run_trajectory_points_episode_y = [], []
        pose,target_pose = [], []
        pose_visual, target_pose_visual = [], []
        velocity_a, global_maps = [], []
        
        # 获取静态障碍物的信息
        obs_list = []
        for i in range(len(self.reset_req.obstacles)):
            obs_msg_dict = {}
            obs_msg_dict['obs_position'] = [self.reset_req.obstacles[i].init_pose.position.x, 
                                        self.reset_req.obstacles[i].init_pose.position.y, 
                                        self.reset_req.obstacles[i].init_pose.position.z]
            obs_msg_dict['obs_orientation'] = [self.reset_req.obstacles[i].init_pose.orientation.x,
                                           self.reset_req.obstacles[i].init_pose.orientation.y,
                                           self.reset_req.obstacles[i].init_pose.orientation.z,
                                           self.reset_req.obstacles[i].init_pose.orientation.w]
            obs_msg_dict['obs_shape'] = self.reset_req.obstacles[i].shape
            obs_msg_dict['obs_size'] = self.reset_req.obstacles[i].size
            obs_list.append(obs_msg_dict)
        
        # 获取机器人的信息
        robot_list = []
        for i in range(len(self.reset_req.robots)):
            robot_msg_dict = {}
            robot_msg_dict['robot_init_position'] = [self.reset_req.robots[i].init_pose.position.x, 
                                               self.reset_req.robots[i].init_pose.position.y, 
                                               self.reset_req.robots[i].init_pose.position.z]
            robot_msg_dict['robot_orientation'] = [self.reset_req.robots[i].init_pose.orientation.x,
                                             self.reset_req.robots[i].init_pose.orientation.y,
                                             self.reset_req.robots[i].init_pose.orientation.z,
                                             self.reset_req.robots[i].init_pose.orientation.w]
            robot_msg_dict['robot_goal'] = [self.reset_req.robots[i].goal.x,
                                      self.reset_req.robots[i].goal.y,
                                      self.reset_req.robots[i].goal.z]
            robot_list.append(robot_msg_dict)
        
        obs_list = obs_list + robot_list
        ...
        for i in range(self.robot_total):
            ...
            pose.append(robot_state.pose)
            target_pose.append(robot_state.target_pose)
            pose_visual.append(robot_state.pose_visual)
            target_pose_visual.append(robot_state.target_pose_visual)
            velocity_a.append(robot_state.velocity_a)
            ...
            refresh_num_episode.append(robot_state.refresh_num_episode)
            run_trajectory_points_episode_x.append(robot_state.run_trajectory_points_episode_x)
            run_trajectory_points_episode_y.append(robot_state.run_trajectory_points_episode_y)
            run_dis_episode.append(robot_state.run_dis_episode)
            vec_states.append(robot_state.state)
            
            sensor_maps.append(self._trans_cv2_sensor_map(robot_state.view_map))
            global_maps.append(self._trans_cv2_global_map(robot_state.global_map))
        ...
        return ImageState(np.array(vec_states),
                          ...
                          np.array(global_maps),
                          np.array(is_collisions),
                          np.array(is_arrives),
                          np.array(lasers) / self.laser_max,
                          np.array(ped_infos),
                          np.array(ped_maps),
                          np.array(refresh_num_episode),
                          np.array(run_dis_episode),
                          np.array(run_trajectory_points_episode_x),
                          np.array(run_trajectory_points_episode_y),
                          step_ds,
                          self.nearby_ped.get(),
                          np.array(velocity_a),
                          np.array(target_pose),
                          np.array(pose),
                          np.array(target_pose_visual),
                          np.array(pose_visual),
                          obs_list
                          )
