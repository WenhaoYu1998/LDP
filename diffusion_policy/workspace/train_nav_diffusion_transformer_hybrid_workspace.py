            ...
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                if (self.epoch % cfg.training.sample_every) == 0:
                    with torch.no_grad():
                        ...
                        obs_dict = batch['obs']
                        gt_lmf = batch['trajectory']['lmf']
                        gt_action = batch['trajectory']['action']
                        gt_reward = batch['trajectory']['reward']
                        gt_rtg = batch['trajectory']['return_to_go']
                        gt_trajectory = torch.cat((gt_lmf, gt_action, gt_reward, gt_rtg), dim=2)
                        
                        result = policy.predict_action(obs_dict)
                        pred_action = result['action_pred']
                        mse = torch.nn.functional.mse_loss(pred_action, gt_trajectory)
                ...
                if (self.epoch % cfg.training.checkpoint_every) == 0:
                    # checkpointing
                    if self.epoch % 200 == 0 and self.epoch != 0:
